...
def evaluate(code, input_buffer=None, init_memory=None, base=256, timeout=1.0,
  ...
  #try:
  #  print(bytearray(input_buffer,"UTF-8"))  
  #except:
  #  print(input_buffer) 
  #try:
  ##  print(bytearray(code,"UTF-8"))  
  #except:
  #  print(code) 
  ...
  try:
    ...
    input_buffer = bytearray(input_buffer,"UTF-8")

  steps,outputlength,outputdata = brainC.run(code,input_buffer,max_steps)
  #if result != None:
  ...
  #print("time %s" % t)
  x = [x for x in outputdata]
  
  return EvalResult(
      output=x,
      success=True,
      failure_reason=Status.SUCCESS,
      steps=steps,
      time=0,
      memory=None,
      program_trace=None)
  ...
  while codeptr < codelen:
    ...
    elif command == '.': output_buffer.append(cells[cellptr])
    elif command == ',': cells[cellptr] = next(input_iter, null_value)
