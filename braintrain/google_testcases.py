...
NUMBER_OF_TESTCASES = 2 # Number of test cases.
MAX_EXECUTION_STEPS = 5000

from bytefuck.bytefuck import Bytefuck
...
def get_examplesource(taskname):
    ...
    def f():
        task_cls, kwargs = task_mapping[taskname]
        task = task_cls(**kwargs)
        while True:
            iogenerator = iter(task.make_io_set())
            while True:
...
class CountCharTask(BaseTask):
  ...
  def __init__(self):
    ...
    self.eos = 0
    self.char = 1
    self.input_type = IOType.string
    self.output_type = IOType.integer
...
class AddTask(BaseTask):
  ...
  def __init__(self, n=16):
    super(type(self), self).__init__()
    self.base = 256
    self.input_type = IOType.integer
    self.output_type = IOType.integer
...
class BooleanLogicTask(BaseTask):
  ...
  def __init__(self):
    super(type(self), self).__init__()
    self.base = 2
    self.input_type = IOType.boolean
    self.output_type = IOType.boolean
    # X(~Z) + (~Y)(~Z) + (~X)YZ
    self._truth_fn = (
...
class KnownCodeBaseTask(BaseTask):
  ...
  def __init__(self, code_solution, make_input_fn, n=100, base=256,
    ...
    self.seed = seed
    self._test_cases = self._test_case_generator(code_solution)

  def _test_case_generator(self, code_solution):
    rand = random.Random()
    bytefuck = Bytefuck()
    while True:
        input_case = bytes(self.make_input_fn(rand))
        result = bytefuck.execute(code_solution, input_case) 
        yield input_case, result
...
class EchoTask(BaseTask):
  ...
  def __init__(self, base, min_length=1, max_length=5):
    ...
    self._io_pairs = self._make_io_examples(50)
...
class JudgeRouteCircleTask(BaseTask):
  ...
  """
  base = 256
  input_type = IOType.integer
  output_type = IOType.integer
...
n = NUMBER_OF_TESTCASES  
task_mapping = {
  'print-hello': (PrintTask, dict(base=27, fixed_string=[8, 5, 12, 12, 15])),
  'print': (PrintIntTask, dict(base=256, fixed_string=[1, 2, 3, 4, 5])),
  'echo': (EchoTask, dict(base=27, min_length=1, max_length=6)),
  'remove-char': (RemoveCharTask, dict(base=256, n=n, min_len=1, max_len=6)),
  'reverse': (ReverseTask, dict(base=256, n=n, min_len=1, max_len=6)),
  'reverse-tune': (ReverseTaskV2, dict(base=256, reward_type='static-bylen')),
  ...
  'circle-route': (JudgeRouteCircleTask, dict(n=100, max_len=32)),
