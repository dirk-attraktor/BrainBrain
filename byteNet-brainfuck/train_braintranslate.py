...
def main():
    ...
    model_name = ""
    if args.model_name:
        model_name = "_%s" % args.model_name
        
    #shutil.rmtree('Data/tb_summaries/translator_model%s' % model_name)
    try:
        os.mkdir('Data/tb_summaries/translator_model%s' % model_name)
    except:
        None
    try:
        os.mkdir("Data/Models/translation_model%s" % model_name)
    except:
        None
        
        
    train_writer = tf.summary.FileWriter('Data/tb_summaries/translator_model%s' % model_name, sess.graph)
        ...
        for bucket_size in bucket_sizes:
            ...
            while (batch_no + 1) * batch_size < len(buckets[bucket_size]):
                ...
                if step % args.sample_every == 0:
                    log_file = open('Data/translator%s_sample.txt' % model_name, 'wb')
